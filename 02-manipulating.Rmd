---
title: "Week 2: Examples for Data Manipulating"
---
...
EPI_standard = (EPI.new-mean(EPI.new)) / sd(EPI.new)
...
epi_data %>%
  mutate(EPI.cat = ifelse(EPI.new >= 50,"Good","Bad")) %>%
  select(code, iso, country, EPI.cat) %>%
  View()
```

# The ifelse() can be nested but if you want more than two levels, 
# but it might be even easier to use case_when() which allows as many 
